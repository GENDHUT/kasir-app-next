...
interface Props {
    ...
    */
    ...
    onChange: (variants: EditSelectedVariant[]) => void;
...
export function EditMenuVariantList({
    ...
    */
    ...
    const availableReusableVariants = useMemo(
        () =>
            reusableMenuVariants.filter((item) => {
                const alreadyUsed = selectedVariantIds.has(item.variantId);
                return !alreadyUsed;
            }),
        [reusableMenuVariants, selectedVariantIds]
    );
    ...
    const availableMasterVariants = useMemo(
        () =>
            masterVariants.filter(
                (item) => !selectedVariantIds.has(item.id)
            ),
        [masterVariants, selectedVariantIds]
    );
    ...
    */
    ...
    const selectedReusableVariant = reusableMenuVariants.find(
    ...
    */
    ...
    function handleReuseVariant() {
        ...
        if (!selectedReusableVariant) {
            alert("Silakan pilih variant yang ingin digunakan.");
            return;
        }
        ...
        */
        ...
        if (selectedVariantIds.has(selectedReusableVariant.variantId)) {
        ...
        */
        ...
        const newVariant: EditSelectedVariant = {
            variantId: selectedReusableVariant.variantId,
            price: Number(selectedReusableVariant.price),
            available: true,
    ...
    */
    ...
    function handleAddNewVariant() {
        ...
        */
        ...
        if (!newVariantPrice.trim()) {
            alert("Harga variant wajib diisi.");
            return;
        }

        const numericPrice = Number(newVariantPrice);
        ...
        */
        ...
        if (selectedVariantIds.has(selectedNewVariantId)) {
            alert("Variant tersebut sudah digunakan pada menu ini.");
            return;
        }
        ...
        */
        ...
        const newVariant: EditSelectedVariant = {
            variantId: selectedNewVariantId,
            price: numericPrice,
            available: true,
            sortOrder: selectedVariants.length,
        };
